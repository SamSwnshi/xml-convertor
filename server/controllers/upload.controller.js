  ...
  try {
    ...
    const accountInformation = Array.isArray(accounts)
        ? accounts.map(account => {
            const addressDetails = account?.CAIS_Holder_Address_Details || {}; // ✅ Fetch address block
    ...
    // ✅ Create and Save New Credit Report
    const newCreditReport = new Credit({
      basicDetails: {
        ...
        pan:
          result?.INProfileResponse?.CAIS_Account?.CAIS_Holder_Details
            ?.Income_TAX_PAN || "Unknown",
        creditScore: score,
      },
      reportSummary,
      accountInformation, // ✅ Added this field
    });
