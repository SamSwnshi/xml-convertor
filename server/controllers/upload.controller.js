...
export const uploadController = async (req, res) => {
  try {
    ...
    const profile = result?.INProfileResponse?.Current_Application?.Current_Application_Details?.Current_Applicant_Details || {};
    ...
    const reportSummary = {
      ...
      last7DaysCredit: parseInt(result?.INProfileResponse?.TotalCAPS_Summary?.TotalCAPSLast7Days) || 0,
    };

  
    const accounts = result?.INProfileResponse?.CAIS_Account?.CAIS_Account_DETAILS;
    const accountInformation = Array.isArray(accounts)
      ? accounts.map(account => {
          ...
          return {
            ...
            address: {
              street: addressDetails?.First_Line_Of_Address_non_normalized || "Unknown",
              city: addressDetails?.City_non_normalized || "Unknown",
              state: addressDetails?.State_non_normalized || "Unknown",
              postalCode: addressDetails?.ZIP_Postal_Code_non_normalized || "Unknown",
            },
    ...
    const newCreditReport = new Credit({
      basicDetails: {
        name: `${profile?.First_Name || "Unknown"} ${profile?.Last_Name || "Unknown"}`,
        mobilePhone: profile?.MobilePhoneNumber || "Unknown",
        pan: result?.INProfileResponse?.CAIS_Account?.CAIS_Holder_Details?.Income_TAX_PAN || "Unknown",
        creditScore: score,
      },
      reportSummary,
      accountInformation,
